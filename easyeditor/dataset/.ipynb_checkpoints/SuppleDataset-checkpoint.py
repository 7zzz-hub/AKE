...
class SuppleDataset:
    def __init__(self, data_path, size, config):
    ...
    def build_data(self):
        ...
        for record in tqdm(data[:self.size]):
            ...
            if gen_image is not None:
                ...
                self.add_qa_pair(sample, "gen2", gen_image, record["gen2_q"], record["gen2_a"])
            
            # loc
            self.add_qa_pair(sample, "Loc_m", m_loc_image, record["m_loc_q"], "_")
            self.add_qa_pair(sample, "Loc_t", t_loc_image, record["t_loc_q"], "_")

...
def compute_icl_multimodal_edit_quality(
    ...
    :return: Dictionary containing rewriting metrics
    """
    vis_root = hparams.image
    # First, unpack rewrite evaluation record.
    target = record["target"]
    prompt = record["prompt"]
    image = vis_processor(Image.open(record["image"]).convert("RGB")).to(dtype=torch.float32).to(hparams.device)

    #rephrase image
    rephrase_image = [Image.open(img_path).convert("RGB") for img_path in record["rephrase_image"]]
    rephrase_image = torch.stack([vis_processor(image).to(dtype=torch.float32) for image in rephrase_image], dim=0).to(hparams.device)

    if "locality_prompt" in record.keys():
        img_loc = [Image.open(img_path).convert("RGB") for img_path in record["img_locality"]]
        img_loc = torch.stack([vis_processor(image).to(dtype=torch.float32) for image in img_loc], dim=0).to(hparams.device)
        loc_q = record["locality_prompt"]
        loc_a = record["locality_ground_truth"]
    if "generality_prompt" in record.keys():
        img_gen = [Image.open(img_path).convert("RGB") for img_path in record["img_generality"]]
        img_gen = torch.stack([vis_processor(image).to(dtype=torch.float32) for image in img_gen], dim=0).to(hparams.device)
        gen_q = record["generality_prompt"]
        gen_a = record["generality_ground_truth"]
    if "generality_image_prompt" in record.keys():
        gen_img = [Image.open(img_path).convert("RGB") for img_path in record["generality_image"]]
        gen_img = torch.stack([vis_processor(image).to(dtype=torch.float32) for image in gen_img], dim=0).to(hparams.device)
        gen_img_q = record["generality_image_prompt"]
        gen_img_a = record['generality_image_ground_truth']
    ...
    if not pre_edit:
        if "rephrase_image" in record.keys():
            rephrase_image_acc = 0.0
            for img in rephrase_image:    
                tmp_rephrase_image_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                                    target, new_fact, img)
                rephrase_image_acc += tmp_rephrase_image_acc
            ret["rephrase_image_acc"] = rephrase_image_acc/len(rephrase_image)

        #generality
        if "generality_prompt" in record.keys():
            generality_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                       gen_a, [f'New Fact: {prompt} {target}\nPrompt: {q}' for q in gen_q], img_gen)     
            ret['generality_acc'] = generality_acc

        
        if "generality_image_prompt" in record.keys():
            generality_image_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                             gen_img_a, [f'New Fact: {prompt} {target}\nPrompt: {q}' for q in gen_img_q], gen_img)
            ret['generality_image_acc'] = generality_image_acc

        #locality
        if "locality_prompt" in record.keys():
            # pre_edit
            pre_text_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, 
                                                                            [''], loc_a, loc_q, img_loc)
            # pose edit
            post_text_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, 
                                                                            icl_examples, loc_a, [f'New Fact: {prompt} {target}\nPrompt: {q}' for q in loc_q], img_loc)
            ...
            locality_acc = sum(post_text_loc_logits_softmax_top_k.view(-1) == 
                            pre_text_loc_logits_softmax_top_k.view(-1))/post_text_loc_logits_softmax_top_k.view(-1).shape[0]
...
def icl_multimodal_lm_eval(
    ...
    if isinstance(x,str):
        ...
        samples = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + f'{t}' for t in x], image)
...
def icl_multimodal_loc_logits(
    ...
    batch = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + f'{t}' for t in x], image) 
    ...
    with torch.no_grad():
        if "qwen" in model.__class__.__name__.lower():
            ...
            outputs = model(input_ids.to(hparams.device), 
                                        images=image.to(hparams.device, dtype=torch.float32))
        else:
            outputs = model(batch)
...
def prepare_multimodal_edit(hparams,
    ...
    if "qwen-vl" in hparams.model_name.lower():
        ...
        if 'blip2' in hparams.model_name.lower():
            prompts_len = [len(tok.encode(prompt, add_special_tokens=False)) for prompt in prompts]
            target = tok(target, add_special_tokens=False, return_tensors="pt",padding=True)["input_ids"]
        else:
            prompts_len = [len(tok.encode(prompt)) for prompt in prompts]
            target = tok(target, return_tensors="pt",padding=True)["input_ids"]
...
def compute_multimodal_edit_quality(model, batch):
    ...
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained("huggingface_cache/blip2-opt-2.7b")
    trg_text = [[tokenizer.decode(idx) for idx in t] for t in targ]
    pred_text = [[tokenizer.decode(idx) for idx in t] for t in pred_ids]

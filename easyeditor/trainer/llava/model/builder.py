...
class ModelVisonArguments:
    vision_tower: Optional[str] = field(default='huggingface_cache/clip-vit-large-patch14-336')
    mm_vision_select_layer: Optional[int] = field(default=-2)
    pretrain_mm_mlp_adapter: Optional[str] = field(default='huggingface_cache/llava-v1.5-7b/mm_projector.bin')
...
def load_pretrained_model(model_path, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs) -> LlavaLlamaForCausalLM:
    ...
    # kwargs = {"device_map": device_map, "attn_implementation": "flash_attention_2", **kwargs}

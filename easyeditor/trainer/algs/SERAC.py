            ...
            if self.config.freeze_cntr:
                ...
                if config.model_class == "BertClassifier":
                    ...
                    if "opt" in config.name:
                        ...
                        self.replacement = OPTForCausalLM.from_pretrained(config.small_name, torch_dtype=torch.bfloat16)
    ...
    def forward(self, *inputs, return_logits_only=True, eps=torch.finfo(torch.float32).eps, pos_pairs=None, **kwargs):
        ...
        if self.config.freeze_cntr:
            ...
            if self.config.model_name == "blip2":
                ...
                # add vision outputs
                image = inputs[0]["image"].to(self.config.device)
                ...
                if image is not None:
                    ...
                    targets = rep_cls_labels.to(image.device)
                ...
                rep_cls_labels = rep_cls_inputs.pop("labels").to(device=self.config.device)
                image = inputs[0]["image"]
                if image is not None:

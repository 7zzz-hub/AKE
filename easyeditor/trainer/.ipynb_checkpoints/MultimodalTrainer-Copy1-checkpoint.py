...
class MultimodalTrainer(BaseTrainer):
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        if config.model_class in ["Blip2OPT", "minigpt4"]:
            ...
            self.vis_processor = transformers.CLIPImageProcessor.from_pretrained("huggingface_cache/clip-vit-large-patch14-336")
    ...
    def edit_step(self, batch, training: bool):
        ...
        with torch.no_grad():
                ...
                for k in batch["loc_1"]:
                    if k=="image":
                        ...
                        if self.model_class == "LLaVA":
                            ...
                            tmp[k] = torch.stack([self.vis_processor(image).to(dtype=torch.float16) for image in images], dim=0)
                    else:
            ...
            if self.model_class == "LLaVA":
                ...
                image = [self.vis_processor(Image.open(img_path).convert("RGB")).to(dtype=torch.float16) for img_path in batch["loc_2"]["image"]]
        ...
        if self.model_class == "LLaVA":
            ...
            image = [self.vis_processor(Image.open(img_path).convert("RGB")).to(dtype=torch.float16) for img_path in batch["edit_inner"]["image"]]
        ...
        with torch.set_grad_enabled(training):
            ...
            l_image_edit /= len(post_image_edit_logits)
            ...
            with torch.no_grad():
                ...
                if self.model_class == "LLaVA":
                    ...
                    image = [self.vis_processor(Image.open(img_path).convert("RGB")).to(dtype=torch.float16) for img_path in batch["gen_1"]["image"]]
                ...
                ###########
                post_edit_logits_2=[]; post_edit_dict_2=[]
                ...
                ###########
                post_edit_logits_3=[]; post_edit_dict_3=[]
                for idx in range(len(batch["gen_3"]['labels'])):
                    tmp = {}
                    for k in batch["gen_3"]:
                        if k=="image":
                            images = [Image.open(img_path).convert("RGB") for img_path in batch["gen_3"][k][idx]]
                            if self.model_class == "LLaVA":
                                tmp[k] = torch.stack([self.vis_processor(image, return_tensors='pt')['pixel_values'] for image in images], dim=0)
                            else:
                                tmp[k] = torch.stack([self.vis_processor(image).to(dtype=torch.float16) for image in images], dim=0)
                        else:
                            tmp[k] = batch["gen_3"][k][idx]
                    post_edit_outputs = edited_model(tmp)
                    if not isinstance(post_edit_outputs, torch.Tensor):
                        post_edit_logits_3.append(post_edit_outputs.logits)
                    else:  
                        post_edit_logits_3.append(post_edit_outputs)
                post_batch_labels_3 = batch["gen_3"]["labels"]

                for _post_edit_logits, _post_batch_labels in zip(post_edit_logits_3, post_batch_labels_3):
                    if _post_edit_logits.shape[1] > _post_batch_labels.shape[1]:
                        post_edit_dict_3.append(self.model.edit_loss_fn(self.config, _post_edit_logits, _post_batch_labels))
                    else:
                        post_edit_dict_3.append(self.model.edit_loss_fn(self.config, _post_edit_logits, _post_batch_labels[:, -_post_edit_logits.shape[1]-1:]))

                ################################ GENERALITY IMAGE ################################
                
                post_gen_image_edit_logits_1=[]; post_gen_image_edit_dict_1=[]
                ...
                ###########
                post_gen_image_edit_logits_2=[]; post_gen_image_edit_dict_2=[]
                ...
                ###########
                post_gen_image_edit_logits_3=[]; post_gen_image_edit_dict_3=[]
            ...
            l_loc_1 /= len(post_base_logits_1)
        ...
        for idx in range(len(pred_ids)):
            if torch.equal(pred_ids[idx][0], trg[idx][0]) and torch.equal(pred_ids[idx][1], trg[idx][1]):
                cnt1 += 1
            if torch.equal(pred_ids[idx][2], trg[idx][2]) and torch.equal(pred_ids[idx][3], trg[idx][3]):
                cnt2 += 1
        info_dict['gen_2_edit_1/acc'] = cnt1 / len(pred_ids) 
        info_dict['gen_2_edit_2/acc'] = cnt2 / len(pred_ids) 
        ##
        cnt1=0; cnt2=0
        pred_ids = [d["pred_ids"] for d in post_edit_dict_3]
        trg = [d["targ_ids"] for d in post_edit_dict_3]
        for idx in range(len(pred_ids)):
            if torch.equal(pred_ids[idx][0], trg[idx][0]):
                cnt1 += 1
            if torch.equal(pred_ids[idx][1], trg[idx][1]):
                cnt2 += 1
        info_dict['gen_3_edit_1/acc'] = cnt1 / len(pred_ids) 
        info_dict['gen_3_edit_2/acc'] = cnt2 / len(pred_ids) 
    ...
    def _inline_validation_log(self, step, stats, start_time, steps):
        ...
        gen_3_acc_1 = f"{stats['gen_3_edit_1/acc_val']:<12.5f}"
        gen_3_acc_2 = f"{stats['gen_3_edit_2/acc_val']:<12.5f}"
        ...
        LOG.info(
          ...
          gen_1_acc: {gen_1_acc} gen_2_acc_1: {gen_2_acc_1} gen_2_acc_2: {gen_2_acc_2} gen_3_acc_1: {gen_3_acc_1} gen_3_acc_2: {gen_3_acc_2} \

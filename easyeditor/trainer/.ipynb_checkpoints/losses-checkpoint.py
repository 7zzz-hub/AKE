...
def multiclass_log_probs(config, pred, targ, shift=False):
    ...
    pred_token = [tokenizer.decode(pred, skip_special_tokens=True) for pred in pred_ids]
    targ_token = [tokenizer.decode(trg, skip_special_tokens=True) for trg in targ]

    n_tokens = mask.float().sum()
    log_prob = (unmasked_log_probs * mask.float()).sum() / n_tokens
    prob = (unmasked_log_probs.exp() * mask.float()).sum() / n_tokens

    acc = sum([pred==targ for pred,targ in zip(pred_token,targ_token)])

...
class BaseTrainer:
    ...
    def load_state(self, checkpoint_path):
        ...
        if self.lr_opt is not None and checkpoint.get("lr_opt") is not None:
            self.lr_opt.load_state_dict(checkpoint["lr_opt"])

        self.global_iter = checkpoint.get("step", 0)
        self.start_time = checkpoint.get("start_time", time.time())
        
        LOG.info(f"✅ Loaded checkpoint, resume at iter {self.global_iter}")
    ...
    def run(self):
        ...
        if self.config.max_epochs is not None:
            ...
            epoch_based_iters = (
                self.config.max_epochs * total_steps_per_epoch
            )
            ...
            if self.config.max_iters is not None:
                ...
                self.config.max_iters = epoch_based_iters
    
            LOG.info(
                f"MAX EPOCH: {self.config.max_epochs}, "
                f"set max iters to {self.config.max_iters}"
            )
    
        # 总 epoch 数
        self.epoches = (
            self.config.max_iters + len(self.train_loader) - 1
        ...
        if not self.config.eval_only and checkpoint_file is not None:
            ...
            start_epoch = self.global_iter // len(self.train_loader)
    
            initial_step = self.global_iter % len(self.train_loader)
            ...
            LOG.info(
                ...
                f"step={initial_step}, "
                f"global_iter={self.global_iter}"
            )
        ...
        for epoch in range(start_epoch, self.epoches):
            ...
            LOG.info(f"Epoch {epoch + 1}/{self.epoches}")
    
            for batch_idx, batch in enumerate(
                tqdm(
                    self.train_loader,
                    ncols=120,
                    desc=f"Epoch {epoch + 1}/{self.epoches}"
                )
            ):
    
                # -------------------------
                # Resume 时仅首个 epoch 跳过
                # -------------------------
                if epoch == start_epoch and batch_idx < initial_step:
                    continue
                ...
                if self.global_iter % self.config.val_interval == 0:
                    ...
                    val_info, _ = self.validate(
                        steps=self.config.val_interval
                    )
                    ...
                    if stopper.should_stop():
                        ...
                        LOG.info(
                            f"No decrease in "
                            f"{self.config.early_stop_key} "
                            f"for "
                            f"{self.config.early_stop_patience} steps"
                        )
        ...
        if not self.config.eval_only:
            ...
            LOG.info(
                f"Training complete after "
                f"{self.global_iter} steps."
            )
        ...
        val_info, records = self.validate(
            log=True,
            steps=val_steps
        )
        ...
        if self.config.results_dir is not None:
            ...
            results_path = (
                f"{model_dir}/"
                f"{cur_time}_{self.config.model_name}_results.json"
            )
    
            records_path = (
                f"{model_dir}/"
                f"{cur_time}_{self.config.model_name}_records.json"
            )
        ...
        with open(records_path, "w") as f:
            ...
            json.dump(
                {"records": convert_tensors(records)},
                f,
                indent=2,
                ensure_ascii=False
            )
    
            LOG.info("Wrote records to:")
            LOG.info(records_path)
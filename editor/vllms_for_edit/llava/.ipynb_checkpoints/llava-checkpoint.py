...
class LlavaForEdit(BaseVLLMForEdit):
    ...
    def __init__(self, model_path:str, device = 'cuda', 
        ...
        self.model = LlavaForConditionalGeneration.from_pretrained(model_path, device_map=device, torch_dtype=torch.bfloat16)
        ...
        def get_llava_llm_inpt(mllm, input_ids, attention_mask, pixel_values):
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                inputs_embeds, attention_mask, labels, position_ids = mllm._merge_input_ids_with_image_features(
                    image_features.permute(1,0,-1), inputs_embeds, input_ids, attention_mask, None
                )
    ...
    def get_llm_outpt(self, llm_inpt, vt_range = None):
        ...
        dtype = next(self.model.language_model.parameters()).dtype
        for k, v in llm_inpt.items():
            if torch.is_tensor(v) and torch.is_floating_point(v):
                llm_inpt[k] = v.to(dtype)

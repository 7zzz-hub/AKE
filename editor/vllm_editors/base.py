...
class VLLMBaseEditorWithTraining(VLLMBaseEditor):
        ...
        for self.train_epoch in range(start_epoch, total_epochs + 1): 
            ...
            for a_batch_samples, samp_n in self.data_generator:
                ...
                if self.train_i % self.log_per_i == 0:
                    ...
                    # Show the three main objectives separately. Generality and
                    # locality may contain multiple buckets, so report their sums.
                    rel_log = log_dict.get('Reliability loss', 0.0)
                    gen_log = log_dict.get('Generality loss', {})
                    loc_log = log_dict.get('Locality loss', {})
                    gen_log = sum(float(v) for v in gen_log.values()) \
                        if isinstance(gen_log, dict) else float(gen_log)
                    loc_log = sum(float(v) for v in loc_log.values()) \
                        if isinstance(loc_log, dict) else float(loc_log)
                    progress_bar.set_postfix({
                        'Rel': '%.4f' % float(rel_log),
                        'Gen': '%.4f' % gen_log,
                        'Loc': '%.4f' % loc_log,
                    })
                if self.train_i % self.save_ckpt_per_i == 0:
                    self.save_ckpt(self.train_i, self.train_epoch, loss, self.ema_loss)
                    last_ckpt_i = self.train_i
                self.train_i += 1 
                progress_bar.update(samp_n)
            progress_bar.close() 

        # Make latest.pt contain the final optimization step even when the save
        # interval does not divide the total number of iterations.
        final_i = self.train_i - 1
        if last_loss is not None and last_ckpt_i != final_i:
            self.save_ckpt(final_i, self.train_epoch, last_loss, self.ema_loss)
    ...
    def validate(self):
        ...
        count = 0
        with torch.no_grad():
            for a_batch_samples, _ in self.val_data_generator:
                loss, _ = self.train_a_batch(a_batch_samples, training=False)
                total_loss += loss
                count += 1
        self.set_train(True)
    ...
    def save_ckpt(self, i:int, epoch:int, loss:float, ema_loss:float = None):
        '''Keep only the latest checkpoint and the checkpoint with the best EMA loss.'''
        ...
        latest_path = os.path.join(self.save_ckpt_dir, 'latest.pt')
        best_path = os.path.join(self.save_ckpt_dir, 'best.pt')

        # Atomic replacement prevents an interrupted save from corrupting the
        ...
        score = float(ema_loss if ema_loss is not None else loss)
        best_score = float('inf')
        if os.path.isfile(best_path):
            best_ckpt = torch.load(best_path, map_location='cpu')
            best_value = best_ckpt.get('ema_loss')
            if best_value is None:
                best_value = best_ckpt.get('loss')
            if best_value is not None:
                best_score = float(best_value)

        if score < best_score:
            best_tmp_path = best_path + '.tmp'
            torch.save(ckpt, best_tmp_path)
            os.replace(best_tmp_path, best_path)
            print('Update best checkpoint: epoch=%d, i=%d, score=%.6f'
                  %(int(epoch), int(i), score))

        # Clean up files made by the previous epoch-based naming scheme.
        keep_names = {'latest.pt', 'best.pt'}
        for name in os.listdir(self.save_ckpt_dir):
            path = os.path.join(self.save_ckpt_dir, name)
            if name not in keep_names and os.path.isfile(path):
                os.remove(path)

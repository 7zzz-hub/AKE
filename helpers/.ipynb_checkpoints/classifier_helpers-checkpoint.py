...
def get_context_model(model, layernum):

    def hook_feature(module, input, output):
        features['pre'] = input[0]
        features['post'] = output

    model.vision_tower.vision_model.encoder.layers[layernum].mlp.fc2.register_forward_hook(hook_feature)
    
    context_model = model

    return context_model
    
def load_classifier(model_path, layernum):

    processor = AutoProcessor.from_pretrained(model_path)
    model = AutoModelForZeroShotImageClassification.from_pretrained(model_path).cuda()
    model.language_model.requires_grad_(False)
    model.multi_modal_projector.requires_grad_(False)
    
    context_model = get_context_model(model, layernum)
    
    trg_model = model.vision_tower.vision_model.encoder.layers[layernum].mlp.fc2
        
    return processor, model, context_model, trg_model
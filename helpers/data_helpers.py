...
def get_val_loader(args, dataset_dir, batch_size=32, workers=8):
...
class AKEVLLMEditData:
    ...
    def _get_img_masks(self, config, processor, image, image_modified, masks, type_modified, text_prompt=None, threshold=128):
        ...
        if config.model_name in ["llava", "blip2"]:
            ...
            modified_tensor = processor(images=image_modified, return_tensors="pt")
    ...
    def get_dataset(self, config) -> List[Dict]:
        ...
        for d in tqdm(dataset):
            ...
            loc_t_image = self._join_img(self.img_root, d.get('m_loc_image'))
            ...
            ################
            loc = {'Loc_m': [], 'Loc_t': []}
            loc_m_q, loc_m_a = d.get('m_loc_q'), "_"
            self._add_data_with_img(loc['Loc_m'], loc_m_image, loc_m_q, loc_m_a)
            loc_t_q, loc_t_a = d.get('t_loc_q'), "_"
